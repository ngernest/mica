...
let parens (p : 'a A.t) : 'a A.t = between (stringP "(") p (stringP ")")
...
let chainl1 (e : 'a A.t) (op : ('a -> 'a -> 'a) A.t) : 'a A.t =
  let rec go (acc : 'a) : 'a A.t =
    A.lift2 (fun f x -> f acc x) op e >>= go <|> return acc
  in
  e >>= fun init -> go init
