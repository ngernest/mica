  ...
  @@ List.fold_left sig_items ~init:[] ~f:(fun acc {psig_desc; psig_loc; _} ->
         ...
         | Psig_value {pval_name; pval_type; pval_loc; _} ->
             ...
             let arg_tys : core_type list =
               remove_last (get_cstr_arg_tys pval_type)
             in
...
let derive_gen_expr ~(loc : Location.t) (ty_cstrs : constructor_declaration list)
    : expression =
      ...
      | _ ->
          ...
          let expr_td =
            mk_adt ~loc ~name:"expr"
              ~cstrs:(List.map ~f:fst (mk_expr_cstrs sig_items))
          in
...
let get_expr_cstrs (mod_ty : module_type) :
  ...
  | Pmty_signature sig_items ->
      get_cstr_metadata (mk_expr_cstrs sig_items)
  | _ ->
      failwith "TODO: get_expr_cstrs"
...
let mk_interp_case_rhs (params : interp_case_rhs_params) : expression =
  ...
  | Some {ppat_desc= Ppat_var {txt= x; loc}; _} ->
      ...
      if not (List.mem x ~set:expr_vars) then
        ...
        let match_rhs =
          get_unary_case_rhs value_cstr mod_name expr_cstr x ~loc
        in
        [%expr
  ...
  | Some {ppat_desc= Ppat_tuple xs; ppat_loc; _} ->
      ...
      if list_is_empty expr_vars then
        let func_args =
          List.map ~f:(fun x -> (Nolabel, pexp_ident_of_string ~loc x)) vars
        in
        pexp_construct ~loc value_cstr
          (Some (pexp_apply ~loc mod_item func_args))
      else
        (* Recursively call [interp] on all variables that have type [expr] *)
        let match_arm =
          get_match_arm ~loc:ppat_loc expr_vars ~abs_ty_parameterized
        in
        let scrutinees = mk_scrutinees expr_vars ~post:(pexp_tuple ~loc) ~loc in
        let func_args =
          List.map
            ~f:(fun x -> pexp_ident_of_string x ~loc)
            (update_expr_arg_names (List.map ~f:add_prime expr_vars) vars)
        in
        let match_rhs =
          get_nary_case_rhs ret_ty_cstr mod_name expr_cstr func_args ~loc
        in
        [%expr
...
let mk_functor ~(loc : location) (arg_name : label option with_loc)
  ...
  let include_decl : structure_item =
    pstr_include ~loc (include_infos ~loc m_expr)
  in
  (* Declaration for the [value] ADT *)
  let val_adt : type_declaration =
    mk_adt ~loc ~name:"value" ~cstrs:(mk_val_cstrs sig_items)
  in
  ...
  let functor_expr : module_expr =
    {pmod_desc= Pmod_structure functor_body; pmod_loc= loc; pmod_attributes= []}
  in
