...
type 'a tree = Empty | Node of 'a tree * 'a * 'a tree [@@deriving sexp, equal]
...
module BSTSet : SetInterface = struct
  (* AF: [Leaf] represents the empty set. [Node (lt, x, rt)] represents the set
     containing [x] & all elements in the sets represented by [lt] and [rt]
     respectively.

     RI: for every [Node (lt, x, rt)], all the values in [lt] are strictly less
     than [x], and all values in [rt] are strictly greater than [x]. *)

  type 'a t = 'a tree [@@deriving sexp, equal]
    ...
    let rec walk node p =
      ...
      | Node (lt, x, rt) ->
        let res_left =
          match lt with
          | Empty -> true
          | Node _ -> walk lt (fun w -> p w && w <= x) in
        let res_right =
          match t with
          | Empty -> true
          | Node _ -> walk rt (fun w -> p w && w >= x) in
        p x && res_left && res_right in
    match t with
    | Empty -> true
    | Node (_, _, _) ->
      (* To introduce a bug, replace with [fun _ -> false] See comment above
  ...
  (* Note that two binary search trees can be equal as sets without having the
     ...
     but don't have the same exact structure. The test case below specifies the
     behavior of `equals`. *)

  (* let equals (t1: 'a t) (t2: 'a t) : bool = (inorder t1) = (inorder t2) *)
end
