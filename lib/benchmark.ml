...
let mica_qc_bench ~(name : string) ~(argv : string list) ~(exe : string) :
    Test.t =
...
let mica_bench ~(name : string) ~(argv : string list) : Test.t =
  ...
  Sys_unix.command @@ String.concat ~sep:" " argv
(* @@ fun () -> Command_unix.run ~argv ~extend cmdLineParser *)

let run_mica_bench : unit =
  (* let open BenchmarkParams in *)
  printf "running mica_bench\n";
  Command_unix.run
  @@ make_command
       [ Test.create ~name:"Sets" @@ fun () -> Sys_unix.command "./bin/main.exe ./lib/sets/SetInterface.ml ./lib/sets/ListSet.ml ./lib/sets/BSTSet.ml
       "
