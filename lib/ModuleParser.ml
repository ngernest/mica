...
let typeParamP : char A.t = quoteP *> lowercaseP
...
let sigP (p : 'a A.t) : 'a A.t = between (stringP "sig") p (stringP "end")

(** Parser for module names, which must start with a capital letter *)
let modNameP : string A.t = identP ~firstCharP:upperCaseP ()

(** Parser for base types *)
let baseTypeP : ty A.t =
  constP "int" Int <|> constP "char" Char <|> constP "bool" Bool
  <|> constP "unit" Unit <|> constP "string" String <|> constP "\'a t" AlphaT
  <|> constP "\'a" Alpha <|> constP "t" T
...
let valDeclP : valDecl A.t =
  ...
  <$> stringP "val" *> lowercaseIdentP
  <* stringP ":" <*> (arrowTypeP <|> typeP)
...
let file_of_string (filename : string) (str : string) : unit =
  Out_channel.with_file ~append:false ~fail_if_exists:false filename
    ~f:(fun output -> Out_channel.output_string output str)
