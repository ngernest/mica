...
module DFA : RegexMatcher = struct 
  ...
  let ex1 : dfa = 
    let open Char in 
    let rec fail = 
      { acceptsEmpty = false ; next = fun _ -> fail } in
    let rec z = 
      { acceptsEmpty = true; 
        next = fun c -> if c = 'b' then z else fail } in
    let y = 
      { acceptsEmpty = true; 
        next = Fn.const fail } in
    let x = 
      { acceptsEmpty = false;  
    ...
    let start = 
      { acceptsEmpty = false; 
        next = fun c -> if c = 'a' then x else fail } in
    start
  ...
  let rec void : dfa = 
    { acceptsEmpty = false; next = fun _ -> void } 
  ...
  let rec star (dfa : dfa) : dfa = 
    { acceptsEmpty = true;
      next = fun c -> cat (next dfa c) (star dfa)
    } 
