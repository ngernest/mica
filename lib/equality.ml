...
and equal_core_type_desc (t1 : core_type_desc) (t2 : core_type_desc) : bool =
  ...
  | Ptyp_tuple xs, Ptyp_tuple ys ->
      equal_core_type_list xs ys
  ...
  | Ptyp_alias (tau1, _), Ptyp_alias (tau2, _) ->
      equal_core_type tau1 tau2
...
let equal_constructor_arguments (xs : constructor_arguments)
  ...
  match (xs, ys) with
  | Pcstr_tuple xss, Pcstr_tuple yss ->
      equal_core_type_list xss yss
