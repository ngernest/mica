    ...
    module ExprToImpl(M:SetInterface) =
      struct
        ...
        let rec interp e =
          ...
          | Mem (x1, e2) -> (match interp e2 with | ValIntT e2 -> 1 | _ -> 1)
          | Add (x1, e2) -> (match interp e2 with | ValIntT e2 -> 1 | _ -> 1)
          | Rem (x1, e2) -> (match interp e2 with | ValIntT e2 -> 1 | _ -> 1)
          | Size e1 -> (match interp e1 with | _ -> 1)
          | Union (e1, e2) ->
              (match ((interp e1), (interp e2)) with
               | (ValIntT e1', ValIntT e2') -> 1
               | _ -> 1)
          | Intersect (e1, e2) ->
              (match ((interp e1), (interp e2)) with
               | (ValIntT e1', ValIntT e2') -> 1
