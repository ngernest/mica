...
(** A signature [S] for finite sets. [S] is implemented by modules [ListSet] & 
    [BSTSet] (see [bin/ListSet.ml] & [bin/BSTSet.ml]). *)
module type S = sig
  type 'a t
...
(** The code below is automatically derived by Mica! *)
include struct
  [@@@ocaml.warning "-60"]
...
(** To collect test metadata for Tyche's visualizations, 
    users can invoke the [Tyche_utils] functor as follows. 
    - Note: this is still work in progress. *)
module MT = Tyche_utils (struct
  include Mica
