...
let () =
  ...
  let test_intoption =
    QC.test_or_error (gen_expr IntOption) ~sexp_of:sexp_of_expr ~f:(fun e ->
      match (I1.interp e, I2.interp e) with
  ...
  let test_unit =
    QC.test_or_error (gen_expr Unit) ~sexp_of:sexp_of_expr ~f:(fun e ->
      match (I1.interp e, I2.interp e) with
  ...
  match
    combine_errors_unit [ test_bool; test_int; test_intoption; test_unit ]
  with
  | Ok ok ->
