...
let constr_decl_eq cd1 cd2 = 
  ...
  begin match (cd1.pcd_args, cd2.pcd_args) with 
...
let uniq_ret_tys_no_dupes () = 
  ...
  Alcotest.check core_ty_list_testable "uniq_ret_tys_no_dupes"
    (List.rev @@ uniq_ret_tys sig_items) [[%type: int]; [%type: string]]  
...
let uniq_ret_tys_singleton () = 
  ...
  Alcotest.check core_ty_list_testable "uniq_ret_tys_singleton" 
    (uniq_ret_tys sig_items) [[%type: int]]
...
let uniq_ret_tys_three_tys () = 
  ...
  Alcotest.check core_ty_list_testable "uniq_ret_tys_three_tys" 
    (List.rev @@ uniq_ret_tys sig_items) [[%type: int]; [%type: string]; [%type: bool]]   
...
let mk_ty_constructors_single_mono_abs_ty () = 
  ...
  Alcotest.check constr_decl_list_testable "mk_ty_constructors_single_mono_abs_ty" 
    (mk_ty_constructors sig_items) [expected] 
...
let mk_ty_constructors_single_poly_abs_ty () = 
  ...
  Alcotest.check constr_decl_list_testable "mk_ty_constructors_single_poly_abs_ty" 
    (mk_ty_constructors sig_items) [expected]     
...
let mk_ty_constructors_two_base () = 
  ...
  let expected = List.map 
    ~f:(fun name -> mk_constructor ~name ~loc:Location.none ~arg_tys:[]) ["Int"; "String"] in 
...
let () = 
  ...
  run "Utils test suite" [
    ...
    ("Tests for [mk_ty_constructors]", [
      ...
      test_case "1 mono abs type" `Quick mk_ty_constructors_single_mono_abs_ty;
      test_case "1 poly abs type" `Quick mk_ty_constructors_single_mono_abs_ty;
