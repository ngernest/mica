...
module Test = struct
    ...
    | Passing ->
      Format.fprintf ppf
        "; The executable under test@,\
         @[<v 1>(executable@ (name %s)@ (modules %s)@ (libraries core)@ (preprocess (pps %s ppx_jane)))@]"
  ...
  let pp_diff_alias_rule ppf name =
    Format.fprintf ppf
      "; Compare the post-processed output to the .expected file@,\
       ...
       (package %s)@,\
       @[<v 1>(action@,\
       @[<hov 2>(diff@ %s.expected@ %s.actual)@])@])@]" ppx_name name name
  ...
  let pp ppf ~kind filename =
    ...
    Format.fprintf ppf
      ...
      module_name (pp_executable ~kind) (name, modules) (pp_rule ~kind)
      (name, module_name) pp_diff_alias_rule name (pp_run_alias_rule ~kind)
      module_name
  ...
  module Suite = struct
    ...
    let create ~kind files =
      let files =
        files |> List.sort ~compare:String.compare |> List.filter ~f:is_test
      in
      { kind; files }
